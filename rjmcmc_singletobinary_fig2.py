...
for ii,p in enumerate(binparams):
    ...
    # theta = [priors[par].draw() for par in params]#
    # theta = actualparams
    alpha = np.array([priors[par].range for par in params])/50.0
    theta1 = []
    theta2 = []
    theta1all=[]
    thetaall=[]
    ...
    theta = proposal(np.array(p[:-1]),covp1,priors,params)
    # theta = np.array(p[:-1])
    #==============================================================================
    #RJMCMC algorithm
    ...
    param1.to_csv("outputs/singletobinary/fig2_singletobinary_%d_output_(single).csv" %(ii))
    param2.to_csv("outputs/singletobinary/fig2_singletobinary_%d_output_(binary).csv" %(ii))
    mdf = pd.DataFrame(m_store,columns=["m"])
    mdf.to_csv("outputs/singletobinary/fig2_singletobinary_%d_mtrace.csv" %(ii))
