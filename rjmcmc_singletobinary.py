...
binparams=[(0.1,15,10,np.pi/3,0.01,0.7,0.4) for d in np.linspace(0.3,0.7,3)]
...
for ii,p in enumerate(binparams):
    ...
    data.to_csv("outputs/singletobinary/singletobinary_data%s.csv" %(parstring))
    ...
    # theta = actualparams
    alpha = np.array([priors[p].range for p in params])/150.0
    theta1 = []
    theta2 = []
    theta1all=[]
    thetaall=[]
    ...
    count2 = 0
    count = 0
    covp1 = (alpha**2)*np.identity(len(theta))
    covp2 = (alpha**2)*np.identity(len(theta))
    ...
    param1.to_csv("outputs/singletobinary/singletobinary_%d_output_(single).csv" %(ii))
    param2.to_csv("outputs/singletobinary/singletobinary_%d_output_(binary).csv" %(ii))
    mdf = pd.DataFrame(m_store,columns=["m"])
    mdf.to_csv("outputs/singletobinary/singletobinary_%d_mtrace.csv" %(ii))
